...
import { api, Character, CheckpointInfo, LibraryImage } from "../api";
...
export default function ImagePlayground({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    let live = true;
    Promise.all([api.getCharacters(), api.getCheckpoints(), api.getLoras()])
      .then(([chars, checkpointData, loraData]) => {
        if (!live) return;
        setCharacters(chars);
  ...
  return <div className="playground theme-images">
    <header className="topbar"><div className="brand"><span className="brand-mark">ai</span><h1>AI influencer playground <b>v3p3</b></h1></div>
      <div className="top-actions">
        <nav className="top-nav"><Link to="/">{t("pg.nav.chat")}</Link><Link className="selected" to="/images">{t("pg.nav.images")}</Link><Link to="/dataset">{t("pg.nav.dataset")}</Link></nav>
    ...
    <div className="workspace image-workspace" style={{ gridTemplateColumns: `225px minmax(320px, 1fr) 6px ${panelWidth}px` }}>
      ...
      <aside className="inspector image-inspector">
        ...
        <div className="ip-form">
          ...
          <label className="auto-label"><input type="checkbox" checked={classify} disabled={!!busy} onChange={e => setClassify(e.target.checked)} /> {t("ip.classify")}</label>
