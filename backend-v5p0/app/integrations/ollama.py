...
CODING_HINTS = ("coder", "codestral", "starcoder", "devstral", "code")


def model_kind(name: str, capabilities: list | None = None) -> str:
    """Classify a model for the chat selector: chat, coding or embedding."""
    lower = name.lower()
    capabilities = capabilities or []
    if "embed" in lower or ("embedding" in capabilities and "completion" not in capabilities):
        return "embedding"
    if any(hint in lower for hint in CODING_HINTS):
        return "coding"
    return "chat"
        ...
        return [
            {
                ...
                "kind": model_kind(m.get("name", ""), m.get("capabilities", [])),
            }
            for m in models
        ]
...
def unload_models() -> list[str]:
    """Drop every model loaded in Ollama (keep_alive=0) and return their names."""
    unloaded: list[str] = []
    try:
