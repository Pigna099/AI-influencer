...
export default function DatasetPlayground({ onLogout }: { onLogout: () => void }) {
  ...
  const classifySource = () => {
    ...
    void perform(t("ds.classifying"), async () => {
      await api.classifyDatasetSource(source.id);
      await loadSources();
    });
  };
  ...
  return <div className="playground theme-dataset">
    <header className="topbar"><div className="brand"><span className="brand-mark">ai</span><h1>AI influencer playground <b>v3p3</b></h1></div>
      <div className="top-actions">
        <nav className="top-nav"><Link to="/">{t("pg.nav.chat")}</Link><Link to="/images">{t("pg.nav.images")}</Link><Link className="selected" to="/dataset">{t("pg.nav.dataset")}</Link></nav>
    ...
    <div className="workspace dataset-workspace">
      <aside className="character-rail">
        ...
        <div className="ip-form">
          ...
          <label className="auto-label"><input type="checkbox" checked={classify} disabled={!!busy} onChange={e => setClassify(e.target.checked)} /> {t("ds.classify")}</label>
