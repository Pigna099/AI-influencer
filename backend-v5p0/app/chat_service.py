...
from .integrations.comfyui import checkpoint_meta, comfyui_available
from .integrations.ollama import OllamaNotAvailable, chat

logger = logging.getLogger(__name__)

PHOTO_REQUEST_PATTERN = re.compile(r"\[(PHOTO|PPV):\s*(.+?)\]", re.IGNORECASE | re.DOTALL)
PHOTO_INSTRUCTIONS = (
    ...
    "photo is sent.\n"
    "PREMIUM PHOTOS (PPV, simulated): when the fan asks for explicit or exclusive content and a free "
    "photo was already sent in this conversation, you can send a locked premium photo by using "
    "[PPV: <English image prompt>] instead. The fan must pay to unlock it; write a teasing caption that "
    "makes clear the photo is locked. Never use [PPV:] for the first photo or when premium content is "
    "disabled for you: in that case use [PHOTO:]."
)
...
def extract_photo_request(answer: str) -> tuple[str, str | None, bool]:
    """Split an optional [PHOTO: ...] or premium [PPV: ...] marker from the reply text.

    Returns (visible_text, scene_prompt, locked).
    """
    match = PHOTO_REQUEST_PATTERN.search(answer)
    if match is None:
        return answer.strip(), None, False
    locked = match.group(1).upper() == "PPV"
    scene = " ".join(match.group(2).split())[:1500]
    clean = PHOTO_REQUEST_PATTERN.sub("", answer)
    clean = re.sub(r"\n{3,}", "\n\n", clean).strip()
    return clean, scene or None, locked


def checkpoint_family(style: str, checkpoint: str | None) -> str:
    """Effective model family: the checkpoint wins, then the style, else real."""
    if checkpoint:
        family = checkpoint_meta(checkpoint)["family"]
        if family != "real":
            return family
    if style == "anime":
        return "anime"
...
def build_prompt_from_profile(
    name: str, profile: dict, scene: str, style: str = "real", family: str | None = None
) -> str:
    family = family or checkpoint_family(style, None)
    appearance = (profile.get("appearance") or profile.get("description") or "").strip()
    if family == "pony":
        tags = settings.chat_pony_style if settings.chat_image_nsfw else "score_9, score_8_up, source_photo"
        return (
            f"score_9, score_8_up, score_7_up, source_photo, {_subject(profile, family)}, {appearance}, "
            f"{scene}, {tags}"
        )
    if family == "anime":
        tags = settings.chat_anime_style if settings.chat_image_nsfw else ANIME_QUALITY
        return f"{ANIME_QUALITY}, {_subject(profile, family)}, {appearance}, {scene}, {tags}"
    tags = (
        settings.chat_real_style
        if settings.chat_image_nsfw
        else "photorealistic, natural skin texture, sharp focus"
    )
    return f"RAW photo, photorealistic, {_subject(profile, family)}, {name}, {appearance}, {scene}, {tags}"


def build_scene_prompt(
    conversation: Conversation, scene: str, style: str = "real", family: str | None = None
) -> str:
    snapshot = conversation.character_snapshot
    return build_prompt_from_profile(
        snapshot.get("name", "the character"), snapshot.get("profile", {}), scene, style, family
    )
...
def build_avatar_prompt(name: str, profile: dict, style: str = "real", family: str | None = None) -> str:
    ...
    if family == "pony":
        ...
        return (
            f"score_9, score_8_up, score_7_up, source_photo, {_subject(profile, family)}, {appearance}, "
            f"selfie portrait, looking at viewer, upper body, bedroom, soft light, {tags}"
        )
    if family == "anime":
        tags = settings.chat_anime_style if settings.chat_image_nsfw else ANIME_QUALITY
        return (
            f"{ANIME_QUALITY}, {_subject(profile, family)}, {appearance}, portrait, looking at viewer, "
    ...
    return (
        f"RAW photo, photorealistic, {_subject(profile, family)}, {name}, {appearance}, selfie portrait, "
        f"looking at viewer, upper body, soft light, {tags}"
    )


def build_negative_prompt(family: str = "real") -> str:
    bases = {
        "pony": settings.chat_pony_negative,
        "anime": settings.chat_anime_negative,
    }
    parts = [bases.get(family, settings.chat_real_negative)]
    if settings.chat_image_nsfw:
...
def human_bursts(answer: str, rng=random) -> list[str]:
    """Human mode: split a reply into short chat messages, with an occasional typo and correction."""
    text = answer.strip()
    if len(text) < 120:
    ...
    if len(sentences) < 3:
        return [text]
    size = max(1, round(len(sentences) / HUMAN_MAX_BURSTS))
    bursts = [" ".join(sentences[index : index + size]) for index in range(0, len(sentences), size)]
    bursts = bursts[:HUMAN_MAX_BURSTS]
    if len(bursts) < 2:
        return [text]
    if rng.random() <= 0.35:
        bursts = _insert_correction(bursts, rng)
    return bursts


def _insert_correction(bursts: list[str], rng) -> list[str]:
    index = rng.randrange(len(bursts))
    words = bursts[index].split()
    if len(words) < 4:
        return bursts
    ...
    chars = list(word)
    middle = len(chars) // 2
    chars[middle - 1], chars[middle] = chars[middle], chars[middle - 1]
    typed = "".join(chars)
    if typed == word:
        return bursts
    words[position] = typed
    bursts[index] = " ".join(words)
    bursts.insert(index + 1, f"*{word}")
    return bursts[: HUMAN_MAX_BURSTS + 1]
...
def generate_reply(conversation: Conversation, content: str, kind: str = "reply", absence_hours: int = 48):
    ...
    if kind == "reply":
        ...
        if kind == "opener":
            ...
            instruction = (
                "Simulation: some time has passed since your last exchange with this fan. Send one short, "
                "spontaneous message to restart the conversation naturally: a small update, a thought or a "
                "playful question. Do not repeat previous messages, add no pressure or sales pitch."
            )
        else:
            instruction = (
    ...
    if images:
        answer, scene, locked = extract_photo_request(answer)
        metrics["photo_requested"] = scene is not None
        if scene:
            metrics["photo_scene"] = scene
            metrics["photo_locked_requested"] = locked
