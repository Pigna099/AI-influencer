...
import { api, Character, CheckpointInfo, Conversation, GpuReport, Memory, Message, OllamaModel, CharacterProfile } from "../api";
...
export default function Playground({ onLogout }: { onLogout: () => void }) {
  ...
  return <div className="playground theme-chat">
    <header className="topbar"><div className="brand"><span className="brand-mark">ai</span><h1>AI influencer playground <b>v3p3</b></h1></div>
      <div className="top-actions"><span className="sandbox-label">{t("pg.sandbox")}</span><nav className="top-nav"><Link className="selected" to="/">{t("pg.nav.chat")}</Link><Link to="/images">{t("pg.nav.images")}</Link><Link to="/dataset">{t("pg.nav.dataset")}</Link></nav><LanguageSwitch /><button onClick={onLogout} disabled={!!busy}>{t("pg.logout")}</button></div></header>
    ...
    <div className="workspace">
      <aside className="character-rail">
        ...
        {character && <div className="ppv-settings">
          <label className="auto-label" title={t("pg.ppvHint")}><input type="checkbox" checked={!!character.ppv_enabled} disabled={!!busy} onChange={e => savePpv(e.target.checked, character.ppv_price_cents ?? 500)} /> {t("pg.ppv")}</label>
      ...
      <main className="main-panel">
        ...
        {tab === "chat" ? <>
          ...
          <ChatPanel conversation={conversation} messages={messages} onSendMessage={send} onUnlockImage={unlockImage} loading={!!busy} busyLabel={busy} elapsed={elapsed} fanName={fan?.name || t("pg.fanFallback")} />
