...
class Settings(BaseSettings):
    ...
    # Pose library: DWPose skeletons extracted from dataset/library images.
    pose_dir: Path = Path("data/poses")
    # Simulated pay-per-view for chat photos (no real payments).
    ppv_enabled: bool = True
    # Realistic replies: schedule the answer after a delay and within activity hours.
    chat_scheduling_enabled: bool = True
    scheduler_enabled: bool = True
    scheduler_poll_seconds: int = 3
    # Human mode: multi-message replies plus spontaneous follow-ups.
    human_mode_proactive_enabled: bool = True
    human_mode_proactive_min_minutes: int = 15
    human_mode_proactive_max_minutes: int = 60
