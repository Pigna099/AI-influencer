# AI influencer playground v3p3 (backend-v3p3)
...
Questa cartella (`backend-v3p3`) è la continuazione di `backend-v3-stage`: stessa base, più il confronto tra varianti di personalità, la duplicazione dei personaggi e alcune rifiniture.
...
```bash
cd /data/data_ssd/pigni/ai-influencer/backend-v3p3
./start.sh
```
...
- **PornMaster Pro SDXL**: dalla fonte originale su Civitai (provenienza variabile sui mirror): vedi `scripts/download_models.py --only pornmaster --civitai-model-id <id>` con `CIVITAI_TOKEN`.
...
- **Cartella sul server**: metti i file in `backend-v3p3/data/imports/<nome>` e indica `<nome>`. Solo percorsi dentro `data/imports` sono accettati.
...
**Utilizzo**:
- **Classifica in attesa**: descrive le immagini ancora `pending`.
