...
from .integrations.comfyui import free_memory
...
app = FastAPI(
    title="AI influencer playground v5p0",
    version="0.5.0",
    lifespan=lifespan,
)
...
app.mount("/ui-assets", StaticFiles(directory=WEBUI_DIR / "ui-assets", check_dir=False), name="webui_assets")
...
@app.post("/api/system/free-vram", tags=["System"], dependencies=[Depends(authorize)])
def free_vram():
    return {"ollama_unloaded": unload_models(), "comfyui": free_memory()}


@app.post("/influencers", status_code=201, tags=["Influencers"], dependencies=[Depends(authorize)])
