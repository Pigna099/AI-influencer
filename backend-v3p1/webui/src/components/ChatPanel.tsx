...
function MessagePhoto({ image, onView, loadingLabel, altTemplate }: { image: ChatImage; onView: (url: string, alt: string) => void; loadingLabel: string; altTemplate: string }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    setUrl("");
    api.fetchBlob(`/api/chat-images/${image.id}/file`)
      .then(blob => {
        if (!alive) return;
    ...
    <div className="message-scroll" ref={scrollRef} role="log" aria-live="polite" aria-relevant="additions">
      ...
      {(showAll ? messages : messages.slice(-60)).map(msg => {
        ...
        return <div key={msg.id} className={`message-row ${msg.role}`}><div className="message-block">
          ...
          <div className="bubble"><p>{content}</p>{!!msg.images?.length && <div className="message-photos">{msg.images.map(image => <MessagePhoto key={image.id} image={image} onView={(url, alt) => setViewer({ url, alt })} loadingLabel={t("chat.photo.loading")} altTemplate={t("chat.photo.alt")} />)}</div>}</div>
