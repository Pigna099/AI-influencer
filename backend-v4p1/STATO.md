...
Cartella server attiva: `/data/data_ssd/pigni/ai-influencer/backend-v4p1` su `192.168.1.60`, utente `john`.
...
Senza ComfyUI attivo la chat resta testuale: il backend verifica la raggiungibilità e non propone foto. La cartella `backend/` (v1) non è più il servizio attivo: eseguire `docker compose` sempre da `backend-v4p1`.
