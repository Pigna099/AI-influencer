...
export type LoraFamily = "real" | "pony" | "anime";
export type LoraStatus = "draft" | "queued" | "training" | "ready" | "failed" | "archived";
...
export interface TrainingJob {
  ...
  progress: { step?: number; total?: number; loss?: number; eta_seconds?: number; message?: string };
  log_path: string | null;
  error: string | null;
  worker: string | null;
...
export interface LoraDatasetDetail extends LoraDataset {
...
export class ApiClient {
  ...
  async classifyDatasetSource(id: string): Promise<{started: boolean; running?: boolean}> {
  ...
  async updateConversation(id: string, data: {title?: string; model?: string; images_enabled?: boolean; human_mode?: boolean}): Promise<Conversation> {
  ...
  async sendMessage(conversationId: string, content: string, scheduled = false): Promise<Message | ScheduledSendResult> {
    return this.request(`/api/conversations/${conversationId}/messages`, {
      method: "POST",
      body: JSON.stringify({content, scheduled}),
    });
  }

  async markConversationRead(conversationId: string): Promise<Conversation> {
    return this.request(`/api/conversations/${conversationId}/read`, { method: "POST" });
  }
  ...
  async setRealism(id: string, data: RealismInput): Promise<Character> {
    return this.request(`/api/characters/${id}/realism`, { method: "PUT", body: JSON.stringify(data) });
  }
  ...
  async getCharacterLoras(characterId: string): Promise<CharacterLora[]> {
    return this.request(`/api/characters/${characterId}/loras`);
  }

  async createLora(characterId: string, data: {name: string; family: LoraFamily; trigger?: string; base_checkpoint?: string; rank: number}): Promise<CharacterLora> {
    return this.request(`/api/characters/${characterId}/loras`, {method: "POST", body: JSON.stringify(data)});
  }

  async patchLora(id: string, data: {name?: string; trigger?: string; rank?: number; status?: string}): Promise<CharacterLora> {
    return this.request(`/api/loras/${id}`, {method: "PATCH", body: JSON.stringify(data)});
  }

  async deleteLora(id: string): Promise<{deleted: boolean; id: string}> {
    return this.request(`/api/loras/${id}`, {method: "DELETE"});
  }

  async activateLora(id: string): Promise<CharacterLora> {
    return this.request(`/api/loras/${id}/activate`, {method: "POST"});
  }

  async queueTraining(loraId: string, data: {images_dir?: string; dataset_id?: string; base_checkpoint?: string; rank?: number; resolution?: number; steps?: number; learning_rate?: number; repeats?: number; batch_size?: number; caption_prefix?: string; save_every?: number; gpu?: number}): Promise<TrainingJob> {
    return this.request(`/api/loras/${loraId}/training-jobs`, {method: "POST", body: JSON.stringify(data)});
  }

  async getTrainingJobs(params: {status?: string; character_id?: string; limit?: number} = {}): Promise<TrainingJob[]> {
    const query = new URLSearchParams();
    if (params.status) query.set("status", params.status);
    if (params.character_id) query.set("character_id", params.character_id);
    query.set("limit", String(params.limit ?? 50));
    return this.request(`/api/training-jobs?${query.toString()}`);
  }

  async getTrainingJob(id: string): Promise<TrainingJob> {
    return this.request(`/api/training-jobs/${id}`);
  }

  async cancelTrainingJob(id: string): Promise<TrainingJob> {
    return this.request(`/api/training-jobs/${id}/cancel`, {method: "POST"});
  }

  async getPoses(params: {tag?: string; active_only?: boolean; limit?: number} = {}): Promise<PoseReference[]> {
    const query = new URLSearchParams();
    if (params.tag) query.set("tag", params.tag);
    if (params.active_only) query.set("active_only", "true");
    query.set("limit", String(params.limit ?? 200));
    return this.request(`/api/poses?${query.toString()}`);
  }

  async extractPose(data: {source: string; source_image_id: string; name?: string; tags?: string[]; description?: string}): Promise<PoseReference> {
    return this.request("/api/poses/extract", { method: "POST", body: JSON.stringify(data) });
  }

  async patchPose(id: string, data: {name?: string; tags?: string[]; description?: string; active?: boolean}): Promise<PoseReference> {
    return this.request(`/api/poses/${id}`, { method: "PATCH", body: JSON.stringify(data) });
  }

  async deletePose(id: string): Promise<{deleted: boolean; id: string}> {
  ...
  async createDataset(characterId: string, data: {family: string; trigger?: string; base_checkpoint?: string; anchor_image_id?: string}): Promise<LoraDataset> {
    return this.request(`/api/characters/${characterId}/datasets`, { method: "POST", body: JSON.stringify(data) });
  }
